...
- イベント詳細ページ
- 候補イベント確認用のローカル管理画面
- GitHub Actions による調査メモ更新と候補 PR の自動作成
...
GitHub Actions は定期実行で Web 調査を行い、条件に合う候補があれば `src/data/candidate_events.ts` を更新した PR を作成します。

- 調査は公式サイト、招聘会社、会場、チケットページを優先します
- SNS は公式アカウントのみを低信頼候補として扱います
- 公開データへは自動反映しません
- PR と `/admin/candidates` の両方で人間が確認します
